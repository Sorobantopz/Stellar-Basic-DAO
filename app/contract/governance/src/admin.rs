...
pub fn initialize(env: &Env, admin: Address) -> Result<(), StellarBasicDAOError> {
    ...
    publish_contract_initialized(
        ...
        stellar_dao_shared::events::EVENT_SCHEMA_VERSION,
        false,
    );
...
pub fn upgrade(
    ...
    stellar_dao_shared::events::publish_contract_upgraded(env, new_wasm_hash, caller);
...
pub fn set_oracle_fee_config(
    env: &Env,
    caller: &Address,
    config: stellar_dao_shared::types::OracleFeeConfig,
) -> Result<(), StellarBasicDAOError> {
    require_any_role(env, caller, &[Role::Admin, Role::Operator])?;
...
pub fn set_platform_wallet(
    ...
    stellar_dao_shared::events::publish_platform_wallet_changed(env, wallet);
    Ok(())
}
