...
use crate::types::{
    ...
    PerAssetFeeConfig, Role, StealthEscrowEntry,
};
...
pub enum DataKey {
    ...
    DisputeExpiryAction,
    // ─── Governance (DAO multisig, Issue #governance-model-v1) ────────────
    /// Vec<Address> signer set for DAO governance (singleton).
    GovernanceSigners,
    /// u32 approval threshold for governance proposals (singleton).
    GovernanceThreshold,
    /// GovernanceProposal stored on-chain, keyed by proposal_id.
    GovernanceProposal(BytesN<32>),
    /// Per-signer approval flag for a specific proposal.
    /// Keyed by (proposal_id, signer_address).
    GovernanceApproval(BytesN<32>, Address),
}
